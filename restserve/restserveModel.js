...
Restaurant.getAllRestaurants = function getAllRestaurants(result) {
    sql.query("select * from (Select * from describes_fooddetails natural join order_details)a join order_table on a.oid = order_table.oid where pending =1 and preparing = 1 and ready =1 ", function (err, res) {
