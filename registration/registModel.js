...
Registration.createRegistration = function createRegistration(data, result) {
    ...
    sql.query("Select * from otpver where cemail = ?", [data.email], function (err,rows, res) {
        if (err) {
           ...
           console.log(rows.otp_id);
            ...
            const output = `
    <p>${rows[0].otp_id} is your one time password for DineSmart account verification.</p>
